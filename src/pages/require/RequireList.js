...
import { REQ_STATUS_CANCEL, REQ_STATUS_DONE } from 'constants/constants';
...
function RequireList({ data, handleClick, isFetching, droppableId, title }) {
  ...
  const processStatus = useCallback(
    (arr) => {
      ...
      if (user.id === arr?.author._id) return droppableId;

      const mem = arr?.to?.find((mem) => mem.info._id === user.id);
      return mem?.status;
    },
    [user.id, isExpired, droppableId],
  );
